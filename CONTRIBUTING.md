...
**`scripts/` example:** every single-APK analysis skill ships a runnable [`apktool run`](https://github.com/android-security-engineer/Apktool-skills) batch script — `security-audit/scripts/audit.json`, `malware-hunt/scripts/hunt.json`, `reverse/scripts/recon.json`, `network-analysis/scripts/network.json`, `dex-deep-dive/scripts/dex.json`, `resource-explorer/scripts/resources.json`, and `signing-verify/scripts/signing.json`. Each executes many analysis commands against a single shared parse of one APK, with per-command error isolation, and is paired with a `references/batch-*.md` that shows how to run or retarget it. Ship deterministic, repetitive command sequences as `scripts/` artifacts rather than as copy-paste heredocs in Markdown — the skill body links to them and shows how to run or `pipe` them at any target. `quick-analysis` (the `analyze` command already does a one-pass) and `compare` (a two-APK `diff`, not a single parse) are the deliberate exceptions.
